...
const Topbar = () => {
    ...
    const clearCorporate = useCorporateStore((state) => state.clearCorporateInfo);
    const Corporate = useCorporateStore((state) => state.corporateInfo);
    const navigate = useNavigate();

    const { fetchCorporateInfo } = useCorporateStore();

    useEffect(() => {
        // Fetch Corporate info when component mounts
        fetchCorporateInfo();
    }, []);
    ...
    const handleRead = async (id) => {
        try {
            ...
            const { data } = await api.post("/Notification/edit-corporate-msg", payload);
    ...
    return (
        <div className="w-full bg-white p-4 flex items-center justify-between">
            <div className="flex items-center ">
                ...
                <p className="lg:text-sm text-[0.6rem]  text-zinc-500">{Corporate?.address || "Complete your kyc registration"}</p>
            ...
            <div className="flex items-center gap-4">
                ...
                >
                    <img src={Corporate.passport ? Corporate.passport : "/images/emptyimage.png"} className="size-8 rounded-full" />

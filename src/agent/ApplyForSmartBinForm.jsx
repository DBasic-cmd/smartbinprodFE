...
const SmartBinApplication = () => {
    ...
    const fetchData = async () => {
        try {
            const { data } = await api.get(`/Agent/all-agent-bin-applications?PageNo=${currentPage}&PageSize=${itemsPerPage}`);
            if (data.succeeded) {
    ...
    const fetchCustomerDetails = async () => {
        try {
            const { data } = await api.get(`/Agent/fetch-customer-address?customerType=${formData.customerType}&customerId=${formData.customerName.value}`);
            if(data.succeded){
                setFormData({
                    ...formData,
                                ...
                                <table className="w-full min-w-[768px] text-sm text-left text-zinc-600">
                                    <thead className="font-light text-zinc-700 uppercase bg-white">
                                        <tr>
                                            ...
                                            <th scope="col" className="px-4 py-3 w-24" role="button" onClick={() => sortBy('customerName')}>
                                                <div className="flex items-center justify-between">
                                            ...
                                            <th scope="col" className="px-4 py-3 w-24" role="button" onClick={() => sortBy('customerType')}>
                                                <div className="flex items-center justify-between">
                            ...
                            <div className="shadow-lg">
                                ...
                                <form onSubmit={submitApplication} className="p-6 bg-white">
                                    <div className="grid grid-cols-1 md:grid-cols-6 gap-x-5 gap-y-6">
                                            ...
                                            <button
                                                type="button"
                                                className={`w-full lg:p-4 p-2 pr-8 border border-zinc-300 rounded-2xl appearance-none focus:ring focus:outline-none focus:ring-green-700 focus:border-green-700 bg-white text-left flex justify-between items-center ${!formData.customerType ? 'text-zinc-400' : 'text-zinc-900'}`}
                                            ...
                                            <div className="relative">
                                                <button
                                                type="button"
                                                className={`w-full lg:p-4 p-2 pr-8 border border-zinc-300 rounded-2xl appearance-none focus:ring focus:outline-none focus:ring-green-700 focus:border-green-700 bg-white text-left flex justify-between items-center ${!formData.customerName ? 'text-zinc-400' : 'text-zinc-900'}`}
                                                onClick={() => setIsCustomerListOpen(!isCustomerListOpen)}
                                                aria-haspopup="listbox"
                                                aria-expanded={isCustomerListOpen}
                                                >
                                                {formData.customerName.text !== '' ? formData.customerName.text : "Select customer name"}

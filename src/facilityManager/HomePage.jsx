...
import FacilityMgrBinDisposalChart from '../components/FacilityMgrBinDisposalChart';
...
const Dashboard = () => {
    ...
    // const fetchFacilityMgrInfo = async () => {
    //     try {
    //        const { data } = await api.get('/facility-managers/account'); 
    //        if (data.succeeded || data.success) {
    //         setDashboardDetails(data.data);
    //        }
    //     } catch (error) {
    //         console.error("Error fetching facility manager info:", error);
    //         setNotification({ type: 'error', message: 'Error fetching facility manager info' });
    ...
    //     try {
    //         const { data } = await api.get("/FacilityMgr/facility-mgr-dashboard");
    //         if (data.succeeded) {
    //             setChartDetails(data.data.disposalChart || []);
    //             setDashboardDetails(data.data);
    //             console.log("chartDetails", data.data.disposalChart)
    //             console.log('fac man info ', dashboardDetails)
    //         }
                ...
                <div className=" bg-zinc-100 min-h-screen flex flex-col flex-1 overflow-y-auto  ">
                    ...
                    <main className="p-8 space-y-6">
                        ...
                        <div className='flex justify-between items-center p-2'>
                            ...
                            <h1 className='text-3xl text-zinc-900/95'>Hello, {facilityMgrInfo?.firstName || "guest"}</h1>
                            ...
                            <div className="w-full sm:w-1/2 lg:w-1/3 px-3 mb-6">
                                <div className="flex bg-white p-6 rounded-2xl  min-h-[230px]  w-full">
                                    <div className=" items-start justify-between w-full">
                                        ...
                                        <div className='flex flex-row justify-between items-center w-full mb-4'>
                                            <div>
                                                <p className="text-zinc-700 font-light">Total Payments Made</p>
                                                <h2 className="text-green-700 text-3xl mt-1">{dashboardDetails.totalPaymentMade}</h2>
                                            </div>
                            ...
                            <div className="w-full sm:w-1/2 lg:w-1/3 px-3 mb-6">
                                <div className="flex bg-white p-6 rounded-2xl  min-h-[230px]  w-full">
                                    <div className=" items-start justify-between w-full">
                                        ...
                                        <div className='flex flex-row justify-between items-center w-full mb-4'>
                                            <div>
                                                <p className="text-zinc-700 font-light">Total Team Members</p>
                                                <h2 className="text-green-700 text-3xl mt-1">{dashboardDetails.totalTeamMembers}</h2>
                                            </div>
                        ...
                        <section className="bg-white p-6 rounded-3xl flex flex-col items-center justify-center">
                            <FacilityMgrBinDisposalChart propData={chartDetails} />
                        </section>
                        <SmartBinTableCard />
                    </main>
                </div>
                            ...
                            <div className="py-6 space-y-4">
                                <label className="px-6 py-4 rounded-lg flex items-center gap-4">
                                    ...
                                    <span className="text-sm font-medium text-zinc-800 flex-grow">
                                        {`Pay from wallet (${parseInt(subscriptionPlans.find((item) => item.id === selectedSubscriptionPlan).price.replace(/[^\d]/g, ''))})`}
                                    </span>
                                    <input
                                        type="radio"
                                        name="paymentMethod"

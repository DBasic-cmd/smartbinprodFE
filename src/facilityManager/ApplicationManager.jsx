...
function AppManager() {
    ...
    const fetchData = async () => {
        try {
            const { data } = await api.get(`/facility-managers/smart-bin/applications/${currentId}`);
    ...
    const getDiscriptions = (item) => {
        if (item.statusName === "DELIVERED") {
            const delivery = formatDelivery(item.date);
            return `${descriptions[item.statusName]} on ${delivery}`;
        }
        return descriptions[item.statusName];
    }
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="font-medium text-zinc-900">{orderDetails.orderID}</p>
                                    </div>
                                </div>
                                <div>
                                    <p className="text-sm text-zinc-500 font-normal">Date Processed</p>
                                    <p className="font-medium text-zinc-900">{formatDate(orderDetails.processDate)}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p className="font-medium text-zinc-900 leading-snug">{orderDetails.customerAddress}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                                    ...
                                    >
                                        ...
                                        <div className={`flex-grow p-6 rounded-[32px] lg:min-w-[750px] mb-3 lg:pl-12 ${getContentBoxClasses(event)}`}>
                                            <div className="flex flex-col sm:flex-row items-start sm:justify-between">
                                                <div>
                                                    <p className="font-medium text-zinc-700">{formatDate(event.date)}</p>
                                                    <p className="text-xs text-zinc-400 mt-0.5">{getDiscriptions(event)}</p>
                                                </div>
                                                <span className={`px-3 py-1 rounded-full text-xs font-medium mt-2 sm:mt-0 whitespace-nowrap ${getStatusBadgeClasses(event.statusName)}`}>
                                                    {event.statusName}
                                                </span>
                                            </div>
                                        </div>

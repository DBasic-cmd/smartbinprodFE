...
const ReceiptPage = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const { data } = await api.get(`/corporate/payments/receipt/${currentId}`);
            if (data.succeeded || data.success) {
                const date = new Date(data.data.transactionDate);
                const newData = {
                    recipientName: data.data.receiptFor,
                    ...
                    currencySymbol: "₦",
                    amountInWords: data.data.amountInWords
                }
